...
@click.option("-of", "--offline", "offline", is_flag=True, help="Search for information not only in local the known_packages")
# TODO: parse URL
...
@click.option('-pm', '--package-manager',
              'pm_name', multiple=False,
              type=click.Choice(PACKAGE_MANAGERS_NAMES, case_sensitive=False))  # Не возможен мультивызов
def install(package_name, pm_name, known_packages_json, all_flag, offline):  # install kit
    '''
    Установить пакет
    '''
    logger.debug(
...
@main.command()
@click.argument('package_name')
@click.option("-a", "--all", "all_flag", is_flag=True, help="Search for information not only the known_packages")
@click.option("-of", "--offline", "offline", is_flag=True, help="Search for information not only in local the known_packages")
# TODO: parse URL
@click.option("-k", "--known-packages-json", type=click.Path(exists=True), help="known_packages.json file")
@click.option('-pm', '--package-manager',
...
def list_command(pm_names, offline, known_packages_json, all_flag):
    ...
    PMs = get_installed_pms(shell=shell)
    if len(pm_names) > 0:
        def pm_fliter(PM): return PM.name in pm_names
        PMs = list(filter(pm_fliter, PMs))
        logger.debug(f"PMs after filtering: {PMs}")
